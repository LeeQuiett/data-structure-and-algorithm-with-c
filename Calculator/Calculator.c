	...
	while (Position < Length)
	{
		...
		else if (Type == RIGHT_PARENTHESIS)
		{
			while (!LLS_IsEmpty(Stack))
			{
				...
				if (Popped->Data[0] == LEFT_PARNTESIS)
				{
		...
		else
		{
			...
			LLS_Push(Stack, LLS_CreatNode(Token));
		}
	}
...
double Calculate(char* PostfixExpression)
{
	...
	while (Read < Length)
	{
		...
		if (Type == OPERAND)
		{
			Node* NewNode = LLS_CreatNode(Token);
		...
		else
		{
			...
			case DIVIED: TempResult = Operator1 / Operator2; break;
			}

			gcvt(TempResult, 10, ResultString); // 문자열로 되어 있는 계산 결과를 소수로 변환
			LLS_Push(Stack, LLS_CreatNode(ResultString));
		}
	}
